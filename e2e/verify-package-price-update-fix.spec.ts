...
test.describe('Package price update via dashboard UI', () => {
  test('navigates to packages list, opens first package edit modal, updates prices in pricing step, and verifies persistence', async ({ page }) => {
    await page.goto('/app/packages')
    await page.waitForLoadState('networkidle')
    console.log('Loaded packages page')

    // Open first package for editing
    ...
    // Set a new price (using 149.99 as a test value)
    await adultPriceInput.fill('149.99')
    console.log('Set adult price to 149.99')
    ...
    await expect(verifyPriceInput).toHaveValue('149.99')
    console.log('✓ Adult price persisted as 149.99')

    console.log('✅ Package price update test completed successfully')
  })
})
