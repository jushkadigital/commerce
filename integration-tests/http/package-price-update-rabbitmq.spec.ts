...
medusaIntegrationTestRunner({
  ...
  testSuite: ({ api, getContainer }) => {
    describe("POST /admin/packages/:id — RabbitMQ integration.product.updated.v1 (real broker)", () => {
      let container: any
      ...
      async function createPackageFixture(overrides: Partial<CreatePackageWorkflowInput> = {}): Promise<any> {
        const input: CreatePackageWorkflowInput = {
          destination: "RabbitMQ Update Test Package",
          description: "Package for real RabbitMQ update event tests",
          duration_days: 2,
          max_capacity: 10,
      ...
      it("updates prices via POST /admin/packages/:id and integration.product.updated.v1 arrives at RabbitMQ", async () => {
        ...
        expect(envelope.payload.id).toBe(pkg.product_id)
        ...
        expect(Number(penPrice.amount)).toBe(600)
      })

      // ─── CASO 2: actualizar sin prices → NO llega integration.product.updated.v1 al exchange real ───
      it("updates package without prices and integration.product.updated.v1 does NOT arrive at RabbitMQ", async () => {
        const pkg = await createPackageFixture()
        ...
        const response = await api.post(
          `/admin/packages/${pkg.id}`,
          { destination: "Updated Package No Prices" },
          { headers: adminHeaders }
        )
        expect(response.status).toBe(200)
      ...
      it("publishes integration.product.updated.v1 with causationId tracing back to the update", async () => {
        ...
        await api.post(
          `/admin/packages/${pkg.id}`,
          { prices: { adult: 700, child: 350, infant: 0, currency_code: "PEN" } },
          { headers: adminHeaders }
        )
        ...
        expect(envelope.metadata.causationId).toContain(pkg.product_id)
      })

      // ─── CASO 4 (causal-link positivo): workflow commitea precio 666 → ese MISMO precio viaja en el evento ───
      it("workflow commits price 666 to DB and the SAME price 666 arrives in the RabbitMQ event", async () => {
        const pkg = await createPackageFixture()
        ...
        const response = await api.post(
          `/admin/packages/${pkg.id}`,
          { prices: { adult: 666, child: 333, infant: 80, currency_code: "PEN" } },
          { headers: adminHeaders }
        )
        expect(response.status).toBe(200)
        ...
        const persistedAdultPrice = rawPrices.find(
          (p: any) => Number(p.amount) === 666 && (p.currency_code === "pen" || p.currency_code === "PEN")
        )
        expect(persistedAdultPrice).toBeDefined()
        expect(Number(persistedAdultPrice.amount)).toBe(666)
        ...
        // ── STEP 3: verify the SAME price (666) that persisted in DB is in the event payload ──
        const payloadVariants = envelope.payload.variants || []
        const adultVariant = payloadVariants.find((v: any) => v.title === "Adult")
        expect(adultVariant).toBeDefined()
        const adultPrices = adultVariant.prices || []
        ...
        expect(Number(eventAdultPrice.amount)).toBe(666)
      ...
      // ─── CASO 5 (causal-link negativo): workflow falla (package inexistente) → NO se publica evento ───
      it("workflow fails for a non-existent package and NO integration.product.updated.v1 is published", async () => {
        // Setup consumer — expect NO event
        const eventPromise = consumeProductUpdatedEvent(10)

        // Call the endpoint with a non-existent package id — the workflow will throw
        // (updatePackageStep tries to retrieve a package that doesn't exist)
        let response: any
        try {
          response = await api.post(
            `/admin/packages/non-existent-package-id-12345`,
