...
const ToursListPage = () => {
  ...
  const [getTour, setGetTour] = useState<null | Tour>(null)


  const handlerUpdateModal = (tour: Tour) => {
    setGetTour(tour)
    setIsModalOpen(true)
  }
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["tour", offset, limit], // La key incluye offset para cachear por página
  ...
  return (
    <Container className="divide-y p-0 h-full flex flex-col">
      ...
      <TourFormModal open={isModalOpen} onOpenChange={handleCloseModal} onSubmit={handleCreateTours} onUpdated={handleUpdateTours} tourToEdit={getTour && getTour} />
    </Container>
  )
}
