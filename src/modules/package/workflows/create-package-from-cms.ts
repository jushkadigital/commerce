...
import { validatePackageStep } from "../../../workflows/steps/create-package-validate"
import { createPackageVariantsStep } from "../../../workflows/steps/create-package-variant"
import { createPackagesStep } from "../../../workflows/steps/create-package-create"
import { validateBookingWindowStep } from "../../../workflows/steps/validate-booking-window"
import { validateCapacityStep } from "../../../workflows/steps/validate-capacity"
import { validateBlockedDatesStep } from "../../../workflows/steps/validate-blocked-dates"
import { buildSlugBase } from "../../../utils/slug"

export type CreatePackageFromCmsWorkflowInput = {
  cms_id: number
  title: string
  slug: string
  destination: string
...
export const createPackageFromCmsWorkflow = createWorkflow(
  "create-package-from-cms",
  (input: CreatePackageFromCmsWorkflowInput) => {
    validatePackageStep({
    ...
    const slug = transform(
      ...
      (data) => {
        const base = buildSlugBase({ destination: data.destination, durationDays: data.duration_days })
    ...
    const productData = transform({
      ...
      const usdRate = 0.27
      const skuPrefix = `${data.input.destination.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${data.cmsId}`
    ...
    const productCreatedEventData = transform(
      { medusaProduct },
      (data: { medusaProduct: any[] }) => ({ id: data.medusaProduct[0]?.id })
    )

    emitEventStep({
      eventName: "product.created",
      data: productCreatedEventData
    }).config({ name: "emit-package-product-created" })
    ...
    }).config({ name: "retrieve-package-result-from-cms" })
...
export default createPackageFromCmsWorkflow