    ...
    try {
        ...
        try {
            ...
            if (!rolePolicies.length) {
                logger.warn("⚠️  Super admin role has no policies assigned. This might indicate an issue with the migration.");
            }
            else {
        ...
        if (successCount > 0) {
            ...
            if (successCount > 0) {
                ...
                logger.info("  3. Verify that the user has access to all admin endpoints");
            }
        }
    }
    catch (error) {
...
async function createSuperAdminRole({ container }) {
    if (!modules_sdk_1.MedusaModule.isInstalled(utils_1.Modules.USER) ||
        ...
        const logger = container.resolve(utils_1.ContainerRegistrationKeys.LOGGER);
        logger.info("⚠️  Required modules (USER, AUTH, RBAC) not installed. Skipping.");
        return;
    }
    try {
